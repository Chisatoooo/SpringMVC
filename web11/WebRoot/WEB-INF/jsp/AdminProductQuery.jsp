...
<html>
  ...
  <% List<Product> productList = (List<Product>) request.getAttribute("productList");
     ...
     String producttext = (String) request.getParameter("producttext");
     if(producttext == null){
     	producttext = "";
     }
 %>
  ...
  <body>
  	<span id="span1">以下为您搜索的商品信息</span>
  	<form id="form1" action="adminQueryProduct.action" method="post" onsubmit="return checkData()">
  		<input id="querytext" type="text" name="producttext" autocomplete="off" value="<%=producttext %>" placeholder=" 请输入商品关键字 （以价格降序排列）"/>
   ...
   <form id="next" action="adminNextQueryProductPage.action" method="post">
       <input id="nextpage" class="page" type="submit" value=<%=nextPage %>>
   </form>
   <label id="number"><%=currentPage %>/${allPage}</label>
   <form id="previous" action="adminPreviousQueryProductPage.action" method="post">
